...
internal class TemplateRepository(CosmosDbContext _dbContext) : ITemplateRepository
{
    ...
    private static Template[] GetDefaultTemplates(Language language)
    {
        ...
        return [
            new() { Id = Guid.Empty, Name = "Default", Text = "I want to write {Context:text}" },
            new() { Id = Guid.Empty, Name = "Email Work Collegue", Text = "I want to write an email to a work collegue named {Name:string}. I know him {Knowledge:select:well,barely}. The tone should be {Tone:select:humble,aggressive,frindly}. The content of the mail consideres {Context:text}" }
            ];
    }
}
