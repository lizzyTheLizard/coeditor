...
internal class PromptMessageFactory
{
    ...
    {
        { Language.DE, "Mache einen neuen Vorschlag" }, { Language.EN, "Create a proposal" }
    };

    private readonly Dictionary<Language, string> InitialContextMessage = new()
    {
        { Language.DE, "Der Kontext ist folgender: {0}" }, { Language.EN, "The context is: {0}" }
    };
    ...
    public PromptMessage[] GenerateInitialMessages(Conversation conversation, Profile profile)
    {
        ...
        [
            ...
            new PromptMessage(string.Format(InitialContextMessage[conversation.Language], conversation.Context),
